...
def loadConfig(file=sys.argv[1]):
    '''
...
def run():    
    ...
    rise_time, set_time, high_time, station, sate = predict_pass(Gs.station_position,Gs.horizon,Gs.date_pred,TLE_line)
    timestamps, rise_times, set_times, high_times = get_passes(station,Gs.date_pred,Gs.num_passes,Gs.timestep,Gs.station_position,Gs.horizon,TLE_line)
    azimuths, elevations, timestamps, station, sate = get_az_el(timestamps, station, sate)
    dopplerfreq, RX, TX = calcDopler(timestamps, set_times, high_times,station,sate,Sat.sat_freq,Gs.channel_step,Gs.channelstep)
